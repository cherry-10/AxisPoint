      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 md:py-24 lg:py-32">
        <div className={`grid ${showHeroImage && !isRealPhoto ? 'lg:grid-cols-2' : 'grid-cols-1'} gap-12 items-center`}>
          ...
          >
            <SplitText
              text={title}
              className={`text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold leading-tight tracking-tight block ${isRealPhoto ? 'text-white drop-shadow-lg' : 'text-gray-900 dark:text-white'}`}
              delay={0}
              splitByWords={true}
            />
            ...
            {subtitle && (
              <FadeInUp delay={0.3}>
                <p 
                  className={`text-base sm:text-lg md:text-xl max-w-prose leading-relaxed ${isRealPhoto ? 'text-gray-100 drop-shadow-md' : 'text-gray-600 dark:text-gray-300'}`}
                >
                  {subtitle}
                </p>
              </FadeInUp>
            ...
            {(primaryCta || secondaryCta) && (
              <FadeInUp delay={0.5}>
                <div 
                  className={`flex flex-col sm:flex-row ${isRealPhoto ? '' : !showHeroImage ? 'justify-center' : ''} gap-3 sm:gap-4 mt-6 sm:mt-8`}
                >
                  {primaryCta && (
                    <Link
            ...
            {children && (
              <FadeInUp delay={0.5} className="mt-8 sm:mt-10 md:mt-12">
                {children}
              </FadeInUp>
            )}
          </motion.div>
            ...
            >
              ...
              >
                ...
                <div className="relative z-10 rounded-2xl overflow-hidden shadow-2xl dark:shadow-blue-500/10">
                  <div className="bg-gradient-to-br from-blue-50/50 to-purple-50/50 dark:from-blue-950/20 dark:to-purple-950/20 p-4 sm:p-6 md:p-8 backdrop-blur-sm">
                    <Image
                      src={heroImage}
