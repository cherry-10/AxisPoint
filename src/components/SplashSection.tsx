...
export const SplashSection = ({
  ...
  return (
    <section className={`relative overflow-hidden ${heightClasses[height]} ${className}`}>
      ...
      {/* Content Container */}
      <div className={`relative h-full flex flex-col justify-end pb-12 sm:pb-16 md:pb-24 ${contentPadding[contentPosition]}`}>
        <motion.div 
        ...
        >
          <motion.h2 
            className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold text-white leading-tight max-w-4xl"
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
          ...
          {subtitle && (
            <motion.div 
              className="text-base sm:text-lg md:text-xl text-gray-200 max-w-3xl"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
      ...
      {/* Floating elements */}
      <motion.div 
        className="absolute bottom-6 sm:bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center space-y-2 hidden sm:flex"
        animate={{
          y: [0, -10, 0],
        }}
        transition={{
...
export const FloatingCard = ({
  ...
  return (
    <motion.div
      className={`absolute ${positionClasses[position]} bg-slate-100/90 dark:bg-slate-800/90 backdrop-blur-lg rounded-2xl p-4 sm:p-6 shadow-2xl max-w-xs ${className}`}
